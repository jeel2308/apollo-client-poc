...
const OrderHistoryQuery = graphql`
  query OrderHistoryPageQuery($skip: Int!, $take: Int!) {
    activeCustomer {
      id
      orders(options: { skip: $skip, take: $take, sort: { updatedAt: DESC } }) {
        totalItems
        items {
          id code state createdAt updatedAt totalWithTax currencyCode totalQuantity
...
const ORDER_STATE_COLORS: Record<string, string> = {
  PaymentAuthorized: '#dbeafe', PaymentSettled: '#d1fae5', Shipped: '#e0e7ff',
  Delivered: '#d1fae5', Cancelled: '#fee2e2', ArrangingPayment: '#fef3c7', AddingItems: '#f3f4f6',
};
const ORDER_STATE_TEXT: Record<string, string> = {
  PaymentAuthorized: '#1e40af', PaymentSettled: '#065f46', Shipped: '#3730a3',
  Delivered: '#065f46', Cancelled: '#991b1b', ArrangingPayment: '#92400e', AddingItems: '#374151',
};
...
function OrderHistoryInner({ take, onLoadMore }: { take: number; onLoadMore: () => void }) {
    ...
    <div>
        ...
        <Link key={order.id} to={`/account/orders/${order.code}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: '1.25rem', marginBottom: '1rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <span style={{ fontSize: '0.75rem', padding: '0.2rem 0.5rem', borderRadius: 4, background: ORDER_STATE_COLORS[order.state] ?? '#f3f4f6', color: ORDER_STATE_TEXT[order.state] ?? '#374151' }}>
                {order.state}
              </span>
            </div>
          </div>
      ...
      {orders.items.length < orders.totalItems && (
        <div style={{ textAlign: 'center', marginTop: '1.5rem' }}>
          ...
          >
            {isPending ? 'Loading...' : 'Load More Orders'}
          </button>
        </div>
      )}
    </div>
...
export function OrderHistoryPage() {
  ...
  return (
    <div>
      ...
      <OrderHistoryInner take={take} onLoadMore={() => setTake(t => t + PAGE_SIZE)} />
    </div>
  );
}
