...
const ActiveOrderQuery = graphql`
  query CartPageQuery {
    activeOrder {
      id code state totalQuantity currencyCode
      ...
      shippingLines {
        ...
        priceWithTax discountedPriceWithTax
        discounts { adjustmentSource type description amount amountWithTax }
      }
      lines {
        ...
        productVariant {
          ...
          product { id slug name featuredAsset { id preview } }
        }
...
const AdjustOrderLineMutation = graphql`
  mutation CartPageAdjustOrderLineMutation($lineId: ID!, $quantity: Int!) {
    adjustOrderLine(orderLineId: $lineId, quantity: $quantity) {
      ... on Order { id totalQuantity totalWithTax lines { id quantity linePriceWithTax } }
...
const RemoveOrderLineMutation = graphql`
  mutation CartPageRemoveOrderLineMutation($lineId: ID!) {
    removeOrderLine(orderLineId: $lineId) {
      ... on Order { id totalQuantity totalWithTax lines { id quantity } }
      ... on ErrorResult { errorCode message }
    }
  }
`;
...
export function CartPage() {
  const data = useLazyLoadQuery(ActiveOrderQuery, {}) as any;
              ...
              <div style={{ flex: 1 }}>
                ...
                {line.discounts.length > 0 && (
                  <div style={{ fontSize: '0.75rem', color: '#10b981', marginTop: '0.25rem' }}>
                    {line.discounts.map((d: any, i: number) => <span key={i}>{d.description} </span>)}
                  </div>
                )}
              </div>
