...
/**
 * Checkout queries OrderAddress (no id) and ShippingLine (no id)
 * on both the order summary and address form, hitting id-less types repeatedly.
 */
...
export function CheckoutPage() {
  ...
  const { data, loading } = useQuery(CheckoutOrderQuery);
  const [setAddress, { loading: settingAddress }] = useMutation(SetShippingAddressMutation);
  const [setShipping, { loading: settingShipping }] = useMutation(SetShippingMethodMutation);
  const [transitionOrder, { loading: transitioningOrder }] = useMutation(TransitionOrderMutation);
  ...
  async function handleShippingSubmit() {
    ...
    await setShipping({ variables: { ids: [selectedMethodId] } });
    setStep('confirm');
  }

  async function handlePlaceOrder() {
    const res = await transitionOrder({ variables: { state: 'ArrangingPayment' } });
    ...
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '2rem' }}>
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            ...
            <div style={{ marginLeft: '0.5rem', width: 60, height: 14, background: '#f3f4f6', borderRadius: 4 }} />
            {i < 2 && <div style={{ flex: 1, height: 1, background: '#e5e7eb', margin: '0 0.75rem' }} />}
          </div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
        <div>
          ...
          <div style={{ height: 44, background: '#f3f4f6', borderRadius: 6, marginTop: '0.5rem' }} />
        </div>
        <div style={{ background: '#f9fafb', borderRadius: 8, padding: '1.5rem', height: 'fit-content' }}>
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} style={{ display: 'flex', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ height: 14, background: '#e5e7eb', borderRadius: 4, marginBottom: '0.4rem' }} />
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' }}>
              ...
              {[
                ...
                <div key={field.name} style={{ marginBottom: '1rem' }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '0.5rem', border: '1px solid #e5e7eb', borderRadius: 4, fontSize: '0.9rem', boxSizing: 'border-box' }}
                  />
                </div>
              ))}
              <button
                ...
                style={{ width: '100%', padding: '0.75rem', background: '#1a1a2e', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600 }}
              >
          ...
          {step === 'shipping' && (
            <div>
              ...
              {data?.eligibleShippingMethods.map(method => (
                <label
                  ...
                  style={{
                    ...
                    border: selectedMethodId === method.id ? '2px solid #e94560' : '1px solid #e5e7eb',
                    borderRadius: 6, marginBottom: '0.75rem', cursor: 'pointer',
                  }}
                >
              ...
              <button
                ...
                disabled={!selectedMethodId || settingShipping}
                style={{ width: '100%', padding: '0.75rem', background: '#1a1a2e', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600, marginTop: '1rem' }}
              >
          ...
          {step === 'confirm' && (
            <div>
              ...
              <button
                ...
                disabled={transitioningOrder}
                style={{ width: '100%', padding: '0.75rem', background: '#e94560', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600, fontSize: '1rem' }}
              >
        ...
        {/* Order summary */}
        <div style={{ background: '#f9fafb', borderRadius: 8, padding: '1.5rem', height: 'fit-content' }}>
          <h3 style={{ fontSize: '0.9rem', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#6b7280' }}>
            Order Summary
          </h3>
