...
export function AccountPage() {
  const { data, loading, refetch } = useQuery(ActiveCustomerQuery);
  const [login, { loading: loggingIn }] = useMutation(LoginMutation);
  const [register, { loading: registering }] = useMutation(RegisterMutation);
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await login({ variables: { username: email, password, rememberMe: true } });
    if (res.data?.login.__typename === 'CurrentUser') {
      await refetch();
    } else if (res.data?.login.__typename !== 'CurrentUser') {
      setError((res.data?.login as any)?.message ?? 'Login failed');
    }
  }
  ...
  async function handleRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (res.data?.registerCustomerAccount.__typename === 'Success') {
      ...
      setError((res.data?.registerCustomerAccount as any)?.message ?? 'Registration failed');
    }
  }
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
        {Array.from({ length: 2 }).map((_, i) => (
          <div key={i} style={{ background: '#f9fafb', borderRadius: 8, padding: '1.5rem' }}>
            ...
            <div style={{ width: '70%', height: 14, background: '#e5e7eb', borderRadius: 4, marginBottom: '0.5rem' }} />
      ...
      <div style={{ display: 'flex', gap: '0', marginBottom: '2rem', border: '1px solid #e5e7eb', borderRadius: 6, overflow: 'hidden' }}>
        {(['login', 'register'] as const).map(m => (
          <button
            key={m}
            onClick={() => setMode(m)}
            style={{ flex: 1, padding: '0.75rem', border: 'none', cursor: 'pointer', background: mode === m ? '#1a1a2e' : '#fff', color: mode === m ? '#fff' : '#374151', fontWeight: mode === m ? 600 : 400, textTransform: 'capitalize' }}
          >
